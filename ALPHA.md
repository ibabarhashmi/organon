...
**What the axis discriminates, and what it does not claim.** It discriminates a real clean pool (compound-v3 → TIMELOCK-gated; aave-v3 → UNRESOLVED) from the EOA-admin danger class — and that class is now grounded in a **real rug's real pre-collapse state**: on PAID Network's V1 token proxy, captured at **block 11975000** (a day before its March 2021 collapse) over three free archive endpoints (cross-checked, content-hash `add26eb0…`), the axis rendered the **damning EOA line** — a single key (the ProxyAdmin's EOA owner, one hop out) controlled the upgrade path, exactly the key that was compromised to mint 59M PAID. The live EOA-admin class is extinct among survivors (0 of ~50 mainnet proxies — such pools did not survive), so the live shelf shows only clean/gated subjects. **The axis flags the upgrade-key surface** (who can replace the code, via the 1967 admin slot, resolved to an EOA directly or one owner-hop out). It does **not** predict depegs, oracle failures, market-mechanism collapses, or upgrade authorities held outside the 1967 admin slot. A clean governance line is **never** a verdict of safety; this is a real rug's real chain state, not a prediction. `GET /postmortems` carries this claim verbatim; `data/honesty/governance-claim.json` is its live state.
