...
## THE GROUND-TRUTH SPRINT — make the Precision axis TRUE against reality, not fixtures (X-GROUNDTRUTH; GroundTruth sprint) · `PINS_SHA 3d0ef3bb…`

Precision built the right axis and proved it where reality wasn't reachable. This sprint reaches: the real source, the real proof, the real rug — or says honestly it couldn't. Three moves + the Aligrithm filing + the whole gate (D27 first), all capture-time, no paid provider, content-hashed into the moat. Carried from Precision `d2fa4cdc…` (whose full X-PRECISION contract lives in `data/honesty/precision-pins.json`). Dual-repo, byte-identical (`organon` + `organon-studio`). Full contract in `data/honesty/groundtruth-pins.json` (`groundtruth_pins.test.ts`).

| Clause | The pin |
|---|---|
| **X-GROUNDTRUTH(a) IMPL-REALIZED (PC2, S61)** | the real aave/compound implementations BUILD from their OWN **Sourcify metadata** (the exact solc version + optimizer + evmVersion + remappings + source tree are IN the verified metadata) and are admitted to the static screen ONLY on a **compiled-vs-deployed runtime-bytecode MATCH** under the **pinned bytecode-mask rule** — EXACTLY TWO masks (solc `immutableReferences` + the CBOR metadata tail; the mask is DATA from the artifact, never a logic waiver; a third mask is a Halt). A **MISMATCH → the subject stays UNVERIFIED**, recorded verbatim; analyzing source the chain doesn't run is a fabrication with extra steps. The mechanism is proven on a positive control + a one-byte-off negative control (→ UNVERIFIED); the flagship match is recorded honestly either way. Capture-time only; mass path `{hono, zod}` |
| **X-GROUNDTRUTH(b) IMMUTABLE-PROVEN (PC4, D30, S62)** | the classifier gains a fifth class **IMMUTABLE**, granted on **BYTECODE-CONSTANT PROOF ONLY** — three conditions, ALL-OR-NOTHING at the pinned block: (1) the implementation address is a **constant embedded in the proxy's deployed bytecode**; (2) the 1967 implementation slot is **unused/zero**; (3) **no admin-slot write path** exists in the proxy bytecode. Anything less stays UNRESOLVED. The **disguised-mutable control** (an embedded-looking constant PLUS a live SSTORE write path) MUST classify UNRESOLVED — a fabricated "no upgrade path" is stronger poison than a wrong SAFE. On a proven IMMUTABLE the collapse extends to fold the **proxy-machinery** findings (moot by proof — the machinery is provably inert) under the grammar *"Immutable implementation — no upgrade path exists; the proxy machinery is inert. (Proven at block N.)"*; **business findings SURVIVE** and are **more permanent** (no patch is coming — the Pro register states it). aave is re-run through the proof and classifies IMMUTABLE ONLY if all three hold on-chain — else it STAYS UNRESOLVED (truth over trophy; the census is an outcome, never a target). The class-set + flagship-render change is **D30 Operator-signed** (parked; info/context, no verdict moves) |
| **X-GROUNDTRUTH(c) REAL-RUG-AT-HEIGHT (PC1, S63)** | ONE pinned rug — **PAID Network (V1) `0x8c8687fc…348df`**, a March 2021 compromised-deployer-key upgrade rug (mechanism-match recorded BEFORE the capture: a single key drove the proxy's upgrade function → malicious impl → minted 59.4M PAID) — its pre-collapse governance state captured at the pinned height **block 11975000** via the SAME three reads the live path makes, over **free archive-capable endpoints only** (a BYOK/paid archive key is a cut; a range-scan/indexer is a cut). Content-hashed + re-verifiable against its named endpoint+height, OR — if no free endpoint serves the height — the **HONEST GAP** (the full attempt log) is recorded and the claim wording HELD at clean-vs-synthetic. **No simulation dressed REAL** is the cardinal sin. The discrimination claim's wording **tracks the evidence in BOTH directions** (an upgraded wording without a capture hash is a Halt; a today-wording despite a landed capture is a Halt); the **does-NOT-claim sentence** (the axis flags the upgrade-key surface, not depegs / not upgrade authority outside the 1967 admin slot) is walled everywhere it renders |
| **X-GROUNDTRUTH(+) the Aligrithm due diligence (AL1/AL3–AL6, D31)** | **Aligrithm** (aligrithm.com; Ali H. Askar, CQF) filed **INSPIRATION-ONLY** in the ledger — a solo quant-research publication with real CSCV/PBO depth and ZERO on-chain scope; nothing integrates. The Stamp's load-bearing methods cite their **PRIMARY papers, never the blog** (Bailey & LdP 2014 — DSR, SSRN 2460551; Bailey/Borwein/LdP/Zhu 2016 — PBO, SSRN 2326253; LdP & Lewis 2019 — ONC, DOI 10.1080/14697688.2019.1622311). The **PBO/CSCV companion** is pinned behind the non-drifting **≥ 20–50 trials/family trigger** (the familyN-deflation trigger) with its implementation **ASSERTED ABSENT** (a grep wall; a seeded `pbo.ts` fails the wall) |

**The Precision findings PC1–PC5 / AL1–AL6.** PC1 the discrimination claim stated precisely + upgraded by the archive capture OR held at the honest gap · PC2 the implementation re-point RUN on the real pools · PC3 D27 top-of-package, three sprints running, presented FIRST with the one-line generosity statement · PC4 UNRESOLVED split into IMMUTABLE (reassuring, on proof) vs UNRESOLVED (caution) · PC5 D29's census is ZERO — the cheapest signature. **D30** the IMMUTABLE class + flagship render · **D31** the Aligrithm filing + citations + trigger — both **Operator-signed** (reserved; operatorSigned=false, PARKED). **The catalog grows to S1–S63** (S61 implementation-truth realized · S62 the IMMUTABLE proof wall · S63 the archive-truth wall). The **mass path stays `hono`+`zod`**; the **screen set the conscious 3**; the parked list (proposer/ONC/PBO — trigger pinned, implementation absent / reports / execution / **archive node / general indexer** — the one-subject capture does NOT unpark it / calibration / meta-labeling / Slither-Mythril-Aderyn / everything Aligrithm) **stays parked**. Verdict-path 7 + frozen-core 2 byte-unchanged; the differential + bundle `9c1e7bd8…` + kill-criterion `8b4e094b` byte-stable; the invites remain one Operator sitting — D27 first — away.
