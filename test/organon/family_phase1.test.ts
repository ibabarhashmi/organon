...
test("S150 — the deviation states render whole from the single source (D51, D33, D63, D27 present; PROVENANCE V42/MR20: D80–D86 too)", () => {
  const ids = State.deviations().map((d) => d.id)
  // the four base deviations lead, in order (the single-producer discipline)
